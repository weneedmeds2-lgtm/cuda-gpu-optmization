...
__global__ void matrix_multiply_tensor_cores_kernel(float* matrix_a, float* matrix_b, float* matrix_c, int N) {
    ...
    // FIX: warpM/warpN must be identical for every thread in a warp, since
    // load_matrix_sync/mma_sync/store_matrix_sync are warp-collective ops.
    // Original had warpN depending on threadIdx.x (varies within a warp) and
    // warpM dividing by 32 (meaningless with blockDim.y=2) — both silently
    // broke addressing, uncaught since the original file had no correctness check.
...
// ===============================================
// Combined absolute+relative tolerance. Pure relative tolerance breaks down
// near zero: with signed random inputs, many dot products land close to 0,
// and dividing a tiny rounding difference by a tiny reference value inflates
// the ratio past any reasonable threshold even though the absolute error is
// negligible. absTol should scale roughly with sqrt(N) * per-element noise.
bool verifyClose(const vector<float>& a, const vector<float>& b, float relTol, float absTol) {
    if (a.size() != b.size()) return false;
    for (size_t i = 0; i < a.size(); i++) {
        float diff = fabsf(a[i] - b[i]);
        float allowed = max(absTol, relTol * fabsf(b[i]));
...
int main(int argc, char** argv) {
    ...
    for (int N : sizes) {
        ...
        results.push_back({"GEMM", "cuBLAS SGEMM", N, cublasMs, computeGflops(N, cublasMs), verifyClose(h_out, h_ref, 1e-2f, 0.5f)});

        // WMMA tensor cores (needs N a multiple of 16 for this simple kernel; pad check)
        if (N % 16 == 0) {
            dim3 blockWmma(32, 2);
            // FIX: grid must match the corrected warp math above — blockDim.x=32
            // is exactly one warp (one N-tile per block, not two), blockDim.y=2
            // gives two M-tiles per block.
            dim3 gridWmma((N + WMMA_N - 1) / WMMA_N, (N + (WMMA_M * 2) - 1) / (WMMA_M * 2));
            float wmmaMs = timeKernel([&]() {
                matrix_multiply_tensor_cores_kernel<<<gridWmma, blockWmma>>>(d_a, d_b, d_c, N);
            });
            CUDA_CHECK(cudaMemcpy(h_out.data(), d_c, bytes, cudaMemcpyDeviceToHost));
            // fp16 staging error accumulates roughly with sqrt(N) terms summed;
            // scale absolute tolerance accordingly rather than using a fixed constant.
            float wmmaAbsTol = 0.1f * sqrtf((float)N);
            results.push_back({"GEMM", "WMMA (tensor cores, fp16)", N, wmmaMs, computeGflops(N, wmmaMs), verifyClose(h_out, h_ref, 5e-2f, wmmaAbsTol)});
        } else {
