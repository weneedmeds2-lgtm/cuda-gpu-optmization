...
__global__ void transpose(int* matrix_b, int N, int* matrixbt) {
...
// TILED KERNEL - Fill in the logic inside
__global__ void matrix_multiply_tiled_kernel(int* matrix_a, int* matrix_b, int* matrix_c, int N) {
    ...
    int globalcol = blockIdx.x * TILE_SIZE + threadIdx.x;
    int sum = 0;
    // TODO: Implement tiled logic here
    for (int t = 0; t < (N + TILE_SIZE - 1) / TILE_SIZE; t++) {
        int tilerow = globalrow * N + (t * TILE_SIZE + threadIdx.x);
        int tilecol = (t * TILE_SIZE + threadIdx.y) * N + globalcol;
        if (globalrow < N && (t * TILE_SIZE + threadIdx.x) < N) {
            tile_a[threadIdx.y][threadIdx.x] = matrix_a[tilerow];
        }
        else {
        ...
        if ((t * TILE_SIZE + threadIdx.y) < N && globalcol < N) {
            tile_b[threadIdx.y][threadIdx.x] = matrix_b[tilecol];
        }
        else {
...
int main() {
    ...
    for (int N : test_sizes) {
        ...
        vector<int> h_a(N * N, 1), h_b(N * N, 1), h_my_c(N * N);
        ...
        double peak_gflops = 1000.0; // Update based on your GPU specs
